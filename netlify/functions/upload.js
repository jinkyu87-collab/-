...
export default async (req, context) => {
  ...
  if (!validTeamRows.length) {
    return json(
      { error: "'팀별' 시트에서 유효한 팀 데이터를 찾지 못했습니다. (팀이름, 실적매출은 필수이며 실적매출은 숫자여야 합니다)" },
      400
      );
  }
  ...
  validTeamRows.forEach((r) => {
    teamRowsByName.set(String(r["팀이름"]).trim(), {
      월: month,
  ...
  const newTeamEntries = Array.from(teamRowsByName.values());
