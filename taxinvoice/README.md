...
5. 배포 후 화면의 **"법인(공급자) 관리"**에서 우리 회사(발행법인) 정보를 입력 — 아래 3번 참고

---

## 2. 법인(공급자) 여러 개 등록하기

법인이 여러 개(예: 3개)면, 화면 맨 위 **"법인(공급자) 관리"**에서 각 법인의 코드/상호/사업자번호/대표자/주소/업태/종목/이메일을 입력하고 **"법인 정보 저장"**을 누르세요.
재배포 없이 즉시 반영됩니다 (Netlify Blobs에 저장). 기본으로 `법인A`/`법인B`/`법인C` 3칸이 비어있는 상태로 준비되어 있으니, 정보만 채워넣으면 됩니다. 4개 이상이 필요하면 알려주시면 칸을 늘려드릴게요.

엑셀 업로드 시 **"발행법인"** 칸에 여기서 정한 코드(예: `법인A`)를 정확히 입력하면, 해당 법인 정보로 계산서가 만들어집니다. 등록 안 된 코드를 입력하면 업로드 시 그 행만 오류로 표시됩니다.
...
4. 상태 탭(대기/발행완료/실패)으로 필터링해서 확인, **거래일자 검색**(기간 지정 또는 "이번 달")으로 월별 조회 가능
5. 잘못 올라온 **대기** 건은 "선택 삭제"로 제거 가능 (이미 발행된 건은 삭제 불가 — 국세청 전송 후에는 수정/취소세금계산서로 처리해야 함)
6. 각 행의 **"보기"** 링크 → 계산서 형태로 조회/인쇄 (거래처에서 계산서를 요청할 때 링크 그대로 전달 가능).
   지금은 스마트빌 연동 전이라 우리 시스템이 자체적으로 그려주는 미리보기이고, 정식 국세청 문서는 아닙니다.
   스마트빌 연동 후 발행된 건은 자동으로 정식 조회 링크로 연결됩니다.

### 엑셀 템플릿 컬럼

| 컬럼 | 필수 | 설명 |
|---|---|---|
| 발행법인 | ✅ | "법인(공급자) 관리"에 등록한 코드 (예: `법인A`) |
| 거래처명 | ✅ | 공급받는자 상호 |
...
```
public/index.html                 화면 (법인 관리, 업로드, 대기목록, 발행)
netlify/functions/upload.js       POST /api/tax/upload    - 엑셀 업로드 → 검증 → 대기목록 추가
netlify/functions/list.js         GET  /api/tax/list       - 목록 조회 (status/from/to로 필터)
...
netlify/functions/view.js         GET  /api/tax/view?id=   - 계산서 형태로 조회/인쇄
netlify/functions/suppliers.js    GET/POST /api/tax/suppliers - 법인(공급자) 목록 조회/저장
lib/invoice.js                    행 검증/정규화 + Netlify Blobs 저장소 접근
lib/auth.js                       비밀번호 확인
lib/suppliers.js                  법인(공급자) 여러 개 관리 (Netlify Blobs)
lib/supplier.js                   (구버전 호환용) 환경변수 기반 공급자 1개 — supplier 정보 없는 옛 건의 fallback
